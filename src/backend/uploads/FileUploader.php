...
class FileUploader {
    ...
    public function __construct($file, $targetDir = __DIR__) {
        $this->file = $file;
    ...
    private function createDirectory($subDir) {
        $dir = $this->targetDir . $subDir . '/';
        if (!file_exists($dir)) {
            mkdir($dir, 0777, true);
        }
        return $dir;
    ...
    public function uploadImage($maxSize = '500KB') {
        ...
        $targetDir = $this->createDirectory('images');
    ...
    public function uploadVideo($maxSize = '50MB') {
        ...
        $targetDir = $this->createDirectory('videos');
        $targetFile = $targetDir . $this->uniqueId . '.' . $this->fileType;
        $this->moveFile($targetFile);
    }
    ...
    public function uploadDocument($maxSize = '10MB') {
        ...
        $targetDir = $this->createDirectory('documents');
        $targetFile = $targetDir . $this->uniqueId . '.' . $this->fileType;
        $this->moveFile($targetFile);
    }
}
